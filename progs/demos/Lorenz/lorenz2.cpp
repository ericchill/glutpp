...
double time_step = 0.005;                     /* Time step in the simulation */
double sigma = 10.0, r = 28.0, b = 8.0/3.0 ;  /* Lorenz attactor parameters */
double position[NUM_PATHS][NUM_POINTS][3] ;          /* Path of the red point */
...
double scale = 0.25; //1.0 ;                          /* Scale factor */
...
void key_cb ( unsigned char key, int x, int y )
{
  ...
  case 'r' :  case 'R' :  /* Reset the simulation */
    ...
    for (i = 0; i < NUM_PATHS; i++) {
      for (int j = 0; j < 3; j++) {
	position[i][0][j] = (double)rand() / (double)RAND_MAX ;
      }
    }
    array_index = 0 ;
	...
	memcpy ( position[j][i], position[j][0], 3 * sizeof(double) ) ;
      }
    }
...
void display_cb ( void )
{
  ...
  for (int i = 0; i < NUM_PATHS; i++) {
    double red = ((i + 1) & 1) == 0 ? 0.0 : 1.0;
    double green = ((i + 1) & 2) == 0 ? 0.0 : 1.0;
    double blue = ((i + 1) & 4) == 0 ? 0.0 : 1.0;
    glColor3d ( red, green, blue);
    draw_curve ( array_index, position[i] ) ;
  }
...
void timer_cb ( int value )
{
  ...
  glutTimerFunc ( /*30*/60, timer_cb, 0 ) ;

  if ( animate > 0 )
  {
    if ( new_index == NUM_POINTS ) new_index = 0 ;
    for (int i = 0; i < NUM_PATHS; i++) {
      advance_in_time ( time_step, position[i][array_index], position[i][new_index] ) ;
    }
